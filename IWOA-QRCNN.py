...
def build_model(Pos):
    ...
    conc = Dense(Dense_units1,activation='relu')(conv)
...
#     model.summary()  
    ...
    history=model.fit([train_Xm],train_labelm,callbacks=[earlyStop],epochs=epochs,batch_size=batch_size,validation_data=([valid_X], valid_label),verbose=0,shuffle=False)
...
model_filename='./ex/models_IWOA_QRCNN0025.h5'
def IWOA_QRCNN(objf,lb,ub,dim,SearchAgents_no,Max_iter):
        ...
        for i in range(0,SearchAgents_no):
            ...
            if Fit_WOA[i]< fitness[i]:
                ...
                if fitness[i]<Leader_score:
                    ...
                    print('Leader_pos:',Leader_pos)
                    print('prediction:',prediction)
                    print('The number of iterations:',ite+1)
                    pred_list.append(prediction)
                    model.save_weights(model_filename)
    ...
    print('Total time of IWOA_QRCNN:', t1)
...
if __name__ == '__main__':
    ...
    SearchAgents_no=15
    Max_iter=10
    print('-------------IWOA_QRCNN0025--------------')
    # IWOA_QRCNN( )  
    pred_value=pd.DataFrame(data=(np.array(pred_list).T))
    pred_value.to_csv("./ex/IWOA_QRCNN0025.csv",encoding='utf-8')
