...
from keras.layers import Dense,Dropout,Flatten,Input,concatenate,add,Activation,Bidirectional,CuDNNGRU
from keras.layers.convolutional import Conv1D,MaxPooling1D
import time
import keras.backend as K
from keras.callbacks import EarlyStopping
import matplotlib.pyplot as plt
from attention.attention import Attention  # https://github.com/philipperemy/keras-attention
...
def build_model():      
    ...
    lstm = Bidirectional(CuDNNGRU(GRU_units1, return_sequences=True))(conv)
    lstm = Bidirectional(CuDNNGRU(GRU_units2, return_sequences=True))(lstm)
    
    lstm = Attention()(lstm)
    lstm = Dropout(Dropout_rate)(lstm)
    lstm = Flatten()(lstm)
    conc = Dense(Dense_units1,activation='relu')(lstm)
    conc = Dense(Dense_units2,activation='relu')(conc)
    outp = Dense(24)(conc)
    model = Model(inputs=[inp], outputs=outp)
    model.summary()

    quantile=0.95  # 0.05、0.95、0.025、0.975
    ...
    history=model.fit([train_Xm],train_labelm,callbacks=[earlyStop],epochs=epochs,batch_size=batch_size,validation_data=([valid_X], valid_label),verbose=2,shuffle=False)
...
if __name__ == '__main__':
    ...
    model_filename='./CNNBiGRUAM095.h5'
    print(result)
    model.save_weights(model_filename) 
    pred_value=pd.DataFrame(data=(np.array(pred_list).T))
    pred_value.to_csv("./CNNBiGRUAM095.csv",encoding='utf-8')