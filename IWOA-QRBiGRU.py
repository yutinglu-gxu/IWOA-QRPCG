...
def build_model(Pos):
    ...
    history=model.fit([train_Xm2],train_labelm,callbacks=[earlyStop],epochs=epochs,batch_size=batch_size,validation_data=([valid_X2], valid_label),verbose=0,shuffle=False)
    
    prediction = model.predict([test_X2])
    scaled_prediction = (prediction*(smax-smin)+smin).reshape(-1)
    result=model.evaluate([valid_X2], valid_label)
    
    return result,scaled_prediction,model


model_filename='./ex/models_IWOA_QRBiGRU0025.h5'
def IWOA_QRBiGRU(objf,lb,ub,dim,SearchAgents_no,Max_iter):
    ...
    print('Total time of IWOA_QRBiGRU:', t1)
...
if __name__ == '__main__':
    ...
    SearchAgents_no=15
    Max_iter=10
    print('-------------IWOA_QRBiGRU0025--------------')
    # IWOA_QRBiGRU( )  
    pred_value=pd.DataFrame(data=(np.array(pred_list).T))
    pred_value.to_csv("./ex/IWOA_QRBiGRU0025.csv",encoding='utf-8')
